...
SPEC_HEIGHT = 3
INIT_WAIT_SECS = 1.0
INIT_DISTANCE_M = 20.0
TIMEOUT_S = 60.0
TEACHER_DISTANCE = 90.0 + random.random() * 20.0
...
def main():
    ...
    parser.add_argument("--port", default=2016, help="CARLA server port")
    ...
    def on_success():
        ...
        ## Compute TTC score
        if ttc_list:
            avg_ttc = sum(ttc_list) / len(ttc_list)
        else:
            avg_ttc = 0.0
        ...
        print("avg_ttc={:.2f}s dist={:.2f}m".format(avg_ttc, dist))
        print(
    ...
    def check():
        ...
        if dist < MIN_DIST_M:
            print("FAIL: Too close to coach car (dist < {}m)".format(MIN_DIST_M))
            on_fail()
